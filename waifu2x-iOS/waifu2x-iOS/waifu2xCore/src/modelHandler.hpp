...
namespace w2xc {
    ...
    class Model {
        ...
        private:
            ...
            bool filterWorker(std::vector <cv::Mat> &inputPlanes, std::vector <cv::Mat> &weightMatrices, std::vector <cv::Mat> &outputPlanes, unsigned int beginningIndex, unsigned int nWorks);
        ...
        public:
            ...
            bool filter(std::vector <cv::Mat> &inputPlanes, std::vector <cv::Mat> &outputPlanes);
        };
    ...
    class modelUtility {
        ...
        public:
            static bool generateModelFromJSON(const std::string &fileName, std::vector <std::unique_ptr <Model>> &models);
